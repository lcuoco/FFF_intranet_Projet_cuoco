<html>
	...
	<body><div class = "container">
		...
		<table class =' tablecent table-borderless table-condensed table-hover'>
        	...
        	$resultataff=PdoFFF::$monPdo->query('SELECT * FROM JOUEUR WHERE IDJOUEUR ="'.(int)$_SESSION['joueuramodif'].'"');
        	$affmodif = $resultataff->fetch();
        	$repcat = PdoFFF::$monPdo->query('SELECT * FROM CATEGORIE WHERE IDCATEGORIE = "'.$affmodif['IDCATEGORIE'].'"');
			...
			<tr>
				...
				<td><select name='mcategJoueur' id ='mcategJoueur' >
						...
						<?php
						$resultats=PdoFFF::$monPdo->query('SELECT * FROM CATEGORIE');
