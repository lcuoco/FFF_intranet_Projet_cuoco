      ...
      /**
        ...
        */
         ...
         */
            ...
            * cette fonction permet d'afficher l'historique de transfert d'un joueur 
            *
            * @param $club = club à transférer
            *
            * @return afficlub = contient toutes les info concerant un club
            */
              function AffichClubsderoul()
                {
                    $resultatacc=PdoFFF::$monPdo->query('SELECT * FROM CLUB WHERE IDCLUB ="'.(int)$_SESSION['clubamodif'].'"');
                    $accmodif = $resultatacc->fetch();

                    $repligue=PdoFFF::$monPdo->prepare('SELECT * FROM CLUB WHERE IDCLUB = :idClub');
                    $repligue->bindParam(':idClub', $_SESSION['clubamodif'], PDO::PARAM_INT);
                    $repligue->execute();
                    $ligue = $repligue->fetchAll();
                    return $ligue;
